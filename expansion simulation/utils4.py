...
def find_min_dis(idx1, idx2):
    # find minimum distance |x| or |y|
    x1, y1 = idx1
    x2, y2 = idx2
    return np.minimum(np.abs(x1-x2),np.abs(y1-y2))
...
def find_out_seq(n):
    ...
    for i in range(int(np.ceil(n/2))):
        seq = [idx_pair for idx_pair in idx_pairs if find_min_dis((center,center), idx_pair) == i]
        out_seq.append(seq)
    return out_seq
...
def update_cps(state_start, state_end, ratio_cps):
    ...
    for i in [0, 1, 2, 3]:  # we also have to consider spores! or it's neither rational nor guarantee enough cps
        diff = state_end[i] - state_start[i]  # >0 means more cps
        state_update[4] += diff * ratio_cps[i%2]
...
def update_number_mig(state_update, idx_pair, neighbors, num_migration):
    """
...
def update_mig(idx_pairs, n, state_update, state_init, grid,
    ...
    for idx_pair in idx_pairs:
        ...
        # for debugging, find where there are bacteria
        # if state_init[0][idx_pair] != 0:
        #     print("not empty")
        diff = calcu_diff(state_init, idx_pair, neighbors)
        num_migration = calcu_num_migration(diff, weight, probs_migration, dtype)
        state_update = update_number_mig(state_update, idx_pair, neighbors, num_migration)
...
def update_con_grow(idx_pairs_rand, r, K, state_update, ratio=2):
    ...
    """
    for i in [0, 1]:
        grow = np.ones(shape=state_update[i].shape) * r[i]  # (n,n), times to multiply
        for idx_pair in idx_pairs_rand:
            # rough model. number > a certain ratio of K, just stop growing
            ...
            # grow[idx_pair] *= state_update[5][idx_pair] / (state_update[5][idx_pair] + state_update[i][idx_pair])
...
def update_con_push2(n, grid, K, state_update):
    """
    ...
    """
    idx_pairs_out = find_out_seq(n)
    max_iter = 20
    for i in range(max_iter):
        for i in [0, 1]:
            for idx_pair_list in idx_pairs_out:
                big = [idx_pair for idx_pair in idx_pair_list if
                       state_update[i][idx_pair] > K[i]]  # find the bigger idxes
                for idx_pair in big:  # if big, some goes out
                    neighbors = find_neighbors(idx_pair, n, grid)  # find neighbors
                    state_update[i] = going_out(idx_pair, neighbors, state_update[i][idx_pair] - K[i], state_update[i])
                    state_update[i][idx_pair] = K[i]

        if np.max(state_update[0]) < K[0] * 1.1 or np.max(state_update[1]) < K[1] * 1.1:  # judge the two simultaneously
            return state_update[0], state_update[1], False

    print("After " + str(max_iter) + ", the cells are still too dense to push each other and reach K"
          + "\nProgram will exit.")
    return state_update[0], state_update[1], True
...
def update_con_push(n, grid, idx_pairs_rand, K, state_update):
    ...
    for t in range(max_iter):  # try to iterate until all parallelograms' values < K[i]
        for i in [0, 1]:
            ...
            if big:
                # print(big)
                # for idx_pair in random.shuffle(big):  # if big, some goes out
                for idx_pair in big:
                    neighbors = find_neighbors(idx_pair, n, grid)  # find neighbors
...
def update_con_nutrient(state_update, l, y, p, ratio_BN, dtype):
    ...
    """
    # for debugging
    # print(np.where(state_update[0] < state_update[2] * ratio_BN))  # where to generate spores
    ...
    # update nutrient for BS and No. minus. y[No] < 0
    for i in [0, 1]:
...
def update_con(idx_pairs_rand, n, state_update, grid, dtype,
               params_BS, params_No, ratio_cps, ratio_BN=0.5):
    """
    1 for BS, 2 for No
    ...
    # state_update[0], state_update[2], exit_flag = update_con_push2(n, grid, (K1, K2), state_update[[0, 2]])
...
# %% main function
def stimulation_v4(n, grid='rect_Moore', state_init=None, epoch=10,
...
def get_ticks(max, sep=None):
    # it is sad that this function can't be applied in animate plot
    if sep is None:
        if max < 600:
            sep = 100
        else:
