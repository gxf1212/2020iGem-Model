...
days=1;
% days=30;
t_toxin=1440*days; % after this time, we add toxin. 
t_max=1440*days; % 1 day=1440 min
[N1, N2, Rc, Rn, Rp, time, G1, G2]=numerical_simulation(n1, n2, rc, rn, rp, t_max, t_toxin);
% [N1, N2, Rc, Rn, Rp, time, G1, G2]=numerical_simulation(n1, n2, rc, rn, rp, t_max);

% cannot run with toxin?? not "run out of memory", it's just too slow
% just close python...
...
idx2=t_toxin/step+1:1:t_max/step; % doesn't matter if overlap

subplot(2,2,3)
title('f of B.S');
hold on
% to1=[ones(1, t_toxin/step), toxin(ga1, N1(idx2), N2(idx2))];
% pl3_3f=plot(time, G1./N1/r1./to1, 'o','MarkerSize', 1.5);
...
title('f of Nostoc');
hold on
% to2=[ones(1, t_toxin/step), toxin(ga2, N1(idx2), N2(idx2))];
% pl3_4f=plot(time, G2./N2/r2./to2, 'o','MarkerSize', 1.5);
